...
namespace Nveto_seg
{
    ...
    void Neutron2DCalibr::SetClusterFile(const UInt_t nNeutrons, const TString& filename)
    {
        ...
        TBranch* branch = tree->GetBranch("Nveto_segClusters");
        TClonesArray* clusters = new TClonesArray("R3BNveto_segCluster");
        ...
        for (Int_t ei = 0; ei < nEntries; ei++)
        {
            ...
            {
                R3BNveto_segCluster* cluster = (R3BNveto_segCluster*)clusters->At(ci);
                if (cluster->GetE() > 0.)
                {
                    Etot += cluster->GetE();
                    validClusters++;
    ...
    void Neutron2DCalibr::Draw(const TString& img) const
    {
        ...
        TCanvas* c = new TCanvas("Neutron2DCalibr", "Nveto_seg Neutron2D Calibr", 1000, (fHists.size() + 1) / 2 * 500);
    ...
    void Neutron2DCalibr::WriteParameterFile(const TString& parFile) const
    {
        ...
        R3BNveto_segNeutron2DPar* par = (R3BNveto_segNeutron2DPar*)rtdb->getContainer("R3BNveto_segNeutron2DPar");

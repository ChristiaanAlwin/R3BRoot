// ---------------------------------------------------------------------------------------
// -----                           R3BNveto_segCal2HitPar                              -----
// -----         Copied from veto_seg02/detector/veto_seg/n_gen_det_cosmic1 on 11-2015       -----
// ---------------------------------------------------------------------------------------

#include "R3BNveto_segCal2HitPar.h"
...
/* About numbering:  VETO_SEG is numbered in x from high x to low x
 ...
 * along x.  You may have to deal with this if you have a split veto_seg
...
// const Double_t paddle_spacing = 10.4;                 // cm VETO_SEG parameter
const Double_t paddle_spacing =  5.0;                 // cm Nveto_seg parameter
...
R3BNveto_segCal2HitPar::R3BNveto_segCal2HitPar()
   : FairTask("R3BNveto_segCal2HitPar")
   , fPar(NULL)
   , fveto_segPmt(NULL) {
}

R3BNveto_segCal2HitPar::R3BNveto_segCal2HitPar(const char* name, Int_t iVerbose)
...
InitStatus R3BNveto_segCal2HitPar::Init() {
   ...
   if (NULL == fveto_segPmt) {
      FairLogger::GetLogger()->Fatal(MESSAGE_ORIGIN, "Branch Nveto_segCalData not found");
      return kFATAL;
   }
   ...
   fPar = (R3BNveto_segHitPar*) FairRuntimeDb::instance()->getContainer("Nveto_segHitPar");
...
void R3BNveto_segCal2HitPar::Exec(Option_t* option) {
   if (++fEventNumber % 10000 == 0)
      FairLogger::GetLogger()->Info(MESSAGE_ORIGIN, "[R3BNveto_segCal2HitPar] Event: %8d,    accepted Events: %8d", fEventNumber, nData);

   Int_t nItems = fveto_segPmt->GetEntriesFast();
   ...
   for (Int_t i = 0; i < nItems; i++) {
      R3BNveto_segCalData* pmt = (R3BNveto_segCalData*) fveto_segPmt->At(i);
   ...
   /*Now we now what fPlanes were hit
     ...
     have to deal with so much noise.  Especially VETO_SEG seem to give
   ...
   for (; ;) {
      ...
      x0 -= (fPaddles / 2 - 0.5);
      y0 -= (fPaddles / 2 - 0.5);
//       if (0) {    required for possible VETO_SEG support
//          x0 = -x0;
//          dxdz = -dxdz;
//       }
...
void R3BNveto_segCal2HitPar::FinishEvent() {
   for (Int_t i = 0; i < fPlanes; i++) {
...
void R3BNveto_segCal2HitPar::FinishTask() {
...
// //       }
   ...
   TH1F* h_tdiff = new TH1F("h_veto_seg_diffc", "TDiff vs BarID", fPaddles * fPlanes, 0.5, 0.5 + fPaddles * fPlanes);
   h_tdiff->SetMaximum(200);
   h_tdiff->SetMinimum(-200);
   h_tdiff->GetXaxis()->SetTitle("BarID");
   h_tdiff->GetYaxis()->SetTitle("tdiff [ns]");

   TH1F* h_tsync = new TH1F("h_veto_seg_tsync", "TSync vs BarID", fPaddles * fPlanes, 0.5, 0.5 + fPaddles * fPlanes);
   h_tsync->SetMaximum(100);
   h_tsync->SetMinimum(-100);
   h_tsync->GetXaxis()->SetTitle("BarID");
   h_tsync->GetYaxis()->SetTitle("tsync [ns]");

   TH1F* h_veff = new TH1F("h_veto_seg_veff", "Veff vs BarID", fPaddles * fPlanes, 0.5, 0.5 + fPaddles * fPlanes);
   ...
   for (Int_t pl = 0; pl < fPlanes; pl++) {
      for (Int_t pdl = 0; pdl < fPaddles; pdl++) {
         for (Int_t pm = 0; pm < 2; pm++) {
            R3BNveto_segHitModulePar* syncmodpar = new R3BNveto_segHitModulePar();
...
ClassImp(R3BNveto_segCal2HitPar)

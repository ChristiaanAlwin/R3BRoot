...
bool sparse_sync_pair_llq::solve(uint max_vars,bool add_eqn_for_each_set)
{
  ...
  // But as this would fool the changing-parameter_seg-finder (if not extra
  ...
  */
  ...
  for (uint i = 0; i < vars; i++)
      ...
      if (variable_set_index[i] > 1)
	{
	  ...
	  // trying to follow parameter_seg over time.  Anyhow, we usually
		    ...
		    float accept_diff,float unreasonable_diff)
{
  ...
  // parameter_seg which will trigger our outside parameter change routines

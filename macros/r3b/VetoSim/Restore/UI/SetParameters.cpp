...
void SetParameters()
{
  ...
  Integers->SetBinContent(219,4); 		// Number of primary particles per event [int]. Default = 1
  ...
  Doubles->SetBinContent(212,0.599);		// Lower Bound op particle total energy! [GeV] Default = 0.0
  Doubles->SetBinContent(213,0.601);		// Upper Bound op particle total energy! [GeV] Default = 1.0
  ...
  Integers->SetBinContent(201,1000);   	// Number of events in a single run [int]. Default = 100
  ...
  TObjString* Event_Generator = new TObjString("ascii");	// Decides which software is used to generate the primary particles.
								...
								// Default = "box"
  TObjString* Output_Name = new TObjString("4n1p");   	// Determines the name of the output files of the simulation. Replace the string

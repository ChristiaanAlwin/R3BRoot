...
void SetParameters()
{
  ...
  Doubles->SetBinContent(128,250.0/1.0);	// VETO Scintillator bar width [cm].       Default:   15.6 (NeuLAND)
  ...
  Integers->SetBinContent(4,1); 		// VETO Number of bars within a single plane [int].        Default = 16
  ...
  TObjString* Event_Generator = new TObjString("ascii");	// Decides which software is used to generate the primary particles.
								...
								// Default = "box"
  TObjString* Output_Name = new TObjString("1000MeVTracker");   	// Determines the name of the output files of the simulation. Replace the string
  ...
  Integers->SetBinContent(220,9);		// Total number of runs the simulation will be repeated after adapting a parameter. 4 means 4 runs in total [int]. Default = 1

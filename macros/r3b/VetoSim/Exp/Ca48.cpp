...
void SetParameters()
{
  ...
  // ----------------------------------- TARGET:
  TObjString* Target_Material = new TObjString("CTarget"); // Target Target_Material choice. Replace the string with one of the following options;
  ...
  Integers->SetBinContent(218,1000200480);     	// Particle ID code: particle species [int]. Default = 2112 (neutrons) NOTE: Check if the mass is added at the end of this file!
  Integers->SetBinContent(219,1); 		// Number of primary particles per event [int]. Default = 1
  Doubles->SetBinContent(202,0.499);		// Lower Bound op particle total energy! [GeV] Default = 0.0
  Doubles->SetBinContent(203,0.501);		// Upper Bound op particle total energy! [GeV] Default = 1.0
  ...
  TObjString* Event_Generator = new TObjString("ascii");	// Decides which software is used to generate the primary particles.
								...
								// Default = "box"
  TObjString* Output_Name = new TObjString("Ca48");   	// Determines the name of the output files of the simulation. Replace the string
  ...
  Doubles->SetBinContent(201,5100.0);		// Magnetic field current [A]. Default = 4000.0 (for 1 GeV beams). 1920.0 works nice for 300 MeV beams.
